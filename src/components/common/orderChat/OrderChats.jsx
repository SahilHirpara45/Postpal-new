...
const OrderChats = ({ userActiveData, packageActiveData, closeHistory }) => {
  ...
  // console.log(userActiveData, "userActiveData in chats");
  ...
  useEffect(() => {
    const chatPath = "orderChats/" + packageActiveData?.id;
    ...
    try {
      ...
      await set(newChatEntryRef, {
        ...
        const chatRefOrder = realtimeRef(
          dbRealtime,
          `orderChats/${packageActiveData?.id}`
        );

        update(chatRefOrder, {
          isOrderChatActive: true,
          adminId: user?.id,
          userId: userActiveData.userId,
  ...
  const editMessage = () => {
    ...
    try {
      const chatRef = realtimeRef(
        dbRealtime,
        `orderChats/${packageActiveData?.id}/chats/${editedMessageId}`
      );
  ...
  const deleteMessage = (messageId) => {
    try {
      ...
      const chatRef = realtimeRef(
        dbRealtime,
        `orderChats/${packageActiveData?.id}/chats/${messageId}`
      );
    ...
    <>
          ...
          <Box sx={{ display: "flex", alignItems: "center" }}>
            <Box sx={{ position: "relative" }}>
              ...
              >
                <CircularProgressbar
                  value={
                    userActiveData?.trustScore ? userActiveData?.trustScore : 0
                  }
                  text={`${
                    userActiveData?.trustScore ? userActiveData?.trustScore : 0
                  }`}
                ...
                <Box sx={{ display: "flex", alignItems: "center" }}>
                  ...
                  >
                    ...
                    >
                      ...
                      {userActiveData?.phoneVerified === true ? (
                        <VerifiedIcon
                          sx={{
                            ml: "4px",
                            color: "primary.main",
      ...
      <Box sx={{ padding: "16px 0px 24px 24px" }}>
        <Box sx={{ position: "relative", height: "72vh" }}>
          ...
          <SimpleBar
            autoHide={true}
            scrollableNodeProps={{ ref: chatPanelRef }}
            style={{ height: "68vh" }}
          >
                ...
                return (
                  <Box key={dateKey} sx={{ pr: "24px" }}>
                    ...
                    {messagesInGroup.map((message) => {
                      ...
                      const me = message.sentBy === user?.id;
